...
function displayAgents(agentArray) {
  ...
  agentArray.forEach((agent, index) => {
    ...
    agentCard.className = 'agent-card rounded-3xl shadow-md aspect-square p-7 mb-1 group cursor-pointer w-370 filter grayscale hover:grayscale-0 transition duration-300 agent-collapse';
    agentCard.style.background = getGradient(agent.backgroundGradientColors);

    const agentIntro = document.createElement('div');
    agentIntro.className = 'agent-intro text-center flex items-center justify-between';
    ...
    iconAgent.className = 'w-60 h-60 object-cover mb-2 pl-7 group-hover:scale-110 transition-transform';
    ...
    agentDetails.className = 'agent-details hidden mt-4 text-left flex text-sm space-y-2 opacity-0 transition-opacity duration-500';

    const agentPortDesc = document.createElement('div');
    const bgWrapper = document.createElement('div');
    bgWrapper.className = 'relative w-full h-140 flex items-center justify-center rounded-2xl overflow-hidden';

    const bgAgent = document.createElement('img');
    bgAgent.src = agent.background;
    bgAgent.className = 'absolute inset-0 w-full h-full object-cover z-0 opacity-20';
    bgWrapper.appendChild(bgAgent);

    const portraitAgent = document.createElement('img');
    portraitAgent.src = agent.fullPortraitV2;
    portraitAgent.className = 'relative z-10 w-full h-140 object-contain mx-auto mt-[-100px]';
...
function getGradient(colors) {
  if (!colors || colors.length < 4) return 'linear-gradient(to bottom right, #1f2937, #111827)';
